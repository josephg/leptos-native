...
// ---------------------------------------------------------------------
//
// One static + one closure impl per value type, generated by the
// [`impl_into_maybe_reactive!`] macro. The macro is re-exported so
// each port's dom crate (where backend-specific types like `Color`
// and `NSTextAlignment` are defined) can add impls for its own
// types without the orphan rule getting in the way.

/// Emits `impl IntoMaybeReactive<T> for T` (static) and
/// `impl<F: Fn() -> T> IntoMaybeReactive<T> for F` (closure) for
/// each type in the list. Use from any crate that owns either the
/// type or this trait — orphan rule applies.
#[macro_export]
macro_rules! impl_into_maybe_reactive {
    ($($t:ty),* $(,)?) => {
        $(
            impl $crate::attrs::IntoMaybeReactive<$t> for $t {
                fn into_maybe_reactive(self) -> $crate::attrs::MaybeReactive<$t> {
                    $crate::attrs::MaybeReactive::Static(self)
                }
            }
            impl<F> $crate::attrs::IntoMaybeReactive<$t> for F
            where
                F: Fn() -> $t + Send + 'static,
            {
                fn into_maybe_reactive(self) -> $crate::attrs::MaybeReactive<$t> {
                    $crate::attrs::MaybeReactive::Reactive(::std::boxed::Box::new(self))
                }
            }
        )*
    };
