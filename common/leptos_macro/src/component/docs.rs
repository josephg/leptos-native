//! Doc-comment + unknown-attribute capture for the `#[component]`
//! macro. These types collect the attributes attached to a component
//! function (or its props) and emit them back into the macro output.
...
impl Docs {
    pub fn new(attrs: &[Attribute]) -> Self {
        ...
        // todo fix docs stuff
        const RSX_START: &str = "# ::leptos_platform::view! {";
        const RSX_END: &str = "# };";
...
pub(super) fn is_lint_attr(attr: &Attribute) -> bool {
    let path = &attr.path();
    path.is_ident("allow")
        || path.is_ident("warn")
