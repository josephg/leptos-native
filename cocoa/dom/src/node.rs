...
impl Node {
    ...
    pub fn insert_node(self, child: &Node, marker: Option<&Node>) {
        ...
        match marker {
            ...
            Some(marker) => {
                ...
                crate::layout::insert_child_at(self, *child, child_index);
            }
        }
    }
    ...
    pub fn on_text_keydown(self, cb: impl FnMut(crate::KeyEvent) + 'static) {
        crate::event::on_text_field_keydown(self, cb);
    }

    /// Wire a keyup observer on a text field. No-op otherwise.
    pub fn on_text_keyup(self, cb: impl FnMut(crate::KeyEvent) + 'static) {
...
// ---------------------------------------------------------------------
//
// With the thread-local store, a `Node` is just a generational
// `NodeId` that owns nothing. Capturing it in a handler closure can't
// form a retain cycle, and a stale id resolves to `None`/no-op. So
// `WeakNode` is the same `Copy` id; `upgrade()` checks presence.

/// Non-owning reference to a `Node`. Identical representation to
/// `Node` (a `Copy` id); kept as a distinct type for API parity.
...
// TRANSITIONAL: `Element` / `WeakElement` are kept only so the large
// `leptos_cocoa` call surface keeps compiling during the NodeId
