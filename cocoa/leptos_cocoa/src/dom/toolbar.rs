...
pub struct ToolbarItemRegistration {
    ...
    pub action_target: Option<Retained<event::ActionTarget>>,
...
pub struct ToolbarItem {
    ...
    /// Last [`Icon`] applied via [`Self::set_icon`] (or
    ...
    last_icon: std::rc::Rc<RefCell<Option<Icon>>>,
}
...
impl ToolbarItem {
    ...
    /// Install an `NSImage` directly, bypassing the [`Icon`]
    ...
    /// Set the item's icon from the unified [`Icon`] enum.
...
pub struct SearchToolbarItem {
    ...
    width_constraint: RefCell<
        Option<Retained<objc2_app_kit::NSLayoutConstraint>>,
    >,
}
...
pub fn search_toolbar_item(
    ...
    // Wrap the search field as a Node so existing string-attr /
