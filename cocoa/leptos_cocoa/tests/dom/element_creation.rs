//! Smoke tests for `CocoaNode::create` per tag.
//!
...
fn color_well_value_round_trips() {
    ...
    let Color::Rgba { r, g, b, a } = got else {
...
fn text_color_on_text_field() {
    ...
    let c = Color::from_nscolor(&got).unwrap();
    let Color::Rgba { r, g, b, .. } = c else {
...
fn date_picker_min_max_round_trip() {
    ...
    let got_min = dp.minDate().map(|d| {
        Date::from_nsdate(&d).seconds_since_epoch
    });
