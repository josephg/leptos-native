...
impl GtkNode {
    pub fn create_button() -> (GtkNode, gtk4::Button) {
        let b = gtk4::Button::new();
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(b.clone(), s).with_tag("button");
        (n, b)
    }

    pub fn create_checkbox() -> (GtkNode, gtk4::CheckButton) {
        let c = gtk4::CheckButton::new();
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(c.clone(), s).with_tag("checkbox");
        (n, c)
    }

    /// Wrapping multi-line label. Default left-aligned text — matches
    /// AppKit's `wrappingLabel` default. Wraps on overflow.
    pub fn create_label() -> (GtkNode, gtk4::Label) {
        ...
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(l.clone(), s).with_tag("label");
        (n, l)
    }

    pub fn create_text_field() -> (GtkNode, gtk4::Entry) {
        let e = gtk4::Entry::new();
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(e.clone(), s).with_tag("text_field");
        (n, e)
    }

    pub fn create_secure_text_field() -> (GtkNode, gtk4::PasswordEntry) {
        let e = gtk4::PasswordEntry::new();
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(e.clone(), s).with_tag("secure_text_field");
        (n, e)
    }

    /// Horizontal `gtk::Scale` with the numeric value display off
    /// (the slider is just the thumb + track).
    pub fn create_slider() -> (GtkNode, gtk4::Scale) {
        ...
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(scale.clone(), s).with_tag("slider");
        (n, scale)
    }

    pub fn create_pop_up_button() -> (GtkNode, gtk4::DropDown) {
        let dd = gtk4::DropDown::default();
        let mut s = Style::default();
        s.flex_shrink = 0.0;
        let n = GtkNode::new_from_widget(dd.clone(), s).with_tag("pop_up_button");
        (n, dd)
    }

    /// Horizontal flexbox container (`<hstack>`).
    pub fn create_hstack() -> GtkNode {
        let w = container_widget();
        let mut s = Style::default();
        s.flex_direction = FlexDirection::Row;
        GtkNode::new_from_widget(w, s).with_tag("hstack")
    }

    /// Vertical flexbox container (`<vstack>` / `<stack_view>`).
    pub fn create_vstack() -> GtkNode {
        let w = container_widget();
        let mut s = Style::default();
        s.flex_direction = FlexDirection::Column;
        GtkNode::new_from_widget(w, s).with_tag("vstack")
    }

    /// Bare flexbox container — direction defaults to Row unless the
    /// builder sets it. `<stack>` / `<view>` route here.
    pub fn create_stack() -> GtkNode {
        let w = container_widget();
        GtkNode::new_from_widget(w, Style::default()).with_tag("stack")
    }
