// leptos_meta is web-only: it drives `<head>` tags (`<title>`,
// `<meta>`, `<link>`, `<script>`, `<style>`) and has no analogue on
// native UI targets. Don't depend on this crate from a native-ui
// build.
#![deny(missing_docs)]
#![forbid(unsafe_code)]
